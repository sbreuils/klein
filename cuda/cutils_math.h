...
// float4 multiply across and shift right
inline __device__ float4 kln_dp_bc(float4 a, float4 b){

    return make_float4(dot(a, b));
}

// float4 multiply across and shift right
inline __device__ float4 kln_hi_dp_bc(float4 a, float4 b){
    float4 vectorPartMsk = make_float4(0.0f,1.0f,1.0f,1.0f);
    return make_float4(dot(vectorPartMsk*a, vectorPartMsk*b));
}


// reciprocal sqrt with an additional single Newton-Raphson refinement.
__device__ float4 kln_rsqrt_nr1(float4 a){


    float4 xn = make_float4(rsqrtf( a.x ));
    // float4 axn2 = a*xn*xn;
    // float4 xn3 = three_diff-axn2;

    float4 xn3 = make_float4(3.0f,3.0f,3.0f,3.0f)-(a*xn*xn);
    

    return make_float4(0.5f,0.5f,0.5f,0.5f)*xn*xn3;
}

// reciprocal with an additional single Newton-Raphson refinement.
__device__ float4 kln_rcp_nr1(float4 a){
    float4 xn = make_float4(1.0) / a;
    return xn*(make_float4(2.0f,2.0f,2.0f,2.0f)-(a*xn));
}
